...
import {bind, createState, For, type JSX} from 'gnim';
...
export function registerLockscreenWidget(def: LockscreenWidgetDef): void {
    if (registry.has(def.id)) {
        logger.warn('lockscreen', `Widget "${def.id}" already registered — skipping`);
        return;
    }
    registry.set(def.id, def);
}

export function getLockscreenWidgets(position: 'center' | 'end'): LockscreenWidgetDef[] {
...
export const LockscreenWidgets = ({position}: {position: 'center' | 'end'}) => {
    ...
    return (
        <Gtk.Box orientation={Gtk.Orientation.VERTICAL} spacing={8} halign={Gtk.Align.CENTER}>
            <For each={widgetList}>{(def: LockscreenWidgetDef) => def.render()}</For>
        </Gtk.Box>
    );
};
