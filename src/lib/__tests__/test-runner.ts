...
let currentSuite = ""
const suites: Array<{ suite: string; tests: Array<{ name: string; fn: TestFn; isAsync: boolean }> }> = []
        ...
        if (isAsync) {
          await Promise.race([
            ...
            new Promise<never>((_, reject) =>
              setTimeout(() => reject(new Error(`Timeout after ${ASYNC_TIMEOUT_MS}ms`)), ASYNC_TIMEOUT_MS),
            ),
          ])
        } else {
          fn()
