...
import {entries, findEntry, type ComponentEntry} from './registry';
...
export const PreviewWindow = (props: PreviewWindowProps) => {
    ...
    const buildPropsPanel = () => {
        ...
        if (eProps.length > 0) {
            inner.append(new Gtk.Separator({marginTop: 4, marginBottom: 4}));
        }
        ...
        for (const [key, def] of eProps) {
            ...
            if (builder) {
                inner.append(
                    builder(key, def, () => propsState(), setPropsState)
                );
            }
        }
