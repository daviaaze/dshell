...
export default () => {
  ...
  const clients = createBinding(hyprland, "clients")
    .as(c => toArray<AstalHyprland.Client>(c))
  ...
  const dockItems = createComputed([
    ...
    for (const df of pinned) {
      const appClients = running.filter(c => getDesktopFileForClient(c) === df)
      const isActive = appClients.some(c => c.address === focused?.address)
      items.push({
        desktopFile: df,
        pinned: true,
        clients: appClients,
