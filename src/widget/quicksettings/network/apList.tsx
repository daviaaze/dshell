...
interface ApListProps {
  ...
  setPasswordDialog: (v: { ap: Network.AccessPoint, entry: Gtk.Entry } | null) => void
}

export default ({ wifi, connectingAp, setConnectingAp, setPasswordDialog }: ApListProps) => {
        ...
        <Gtk.Box spacing={8}>
          ...
          <With value={apStatus}>
            {(status: "active" | "connecting" | "idle") => {
              if (status === "active")
                return <Gtk.Image iconName="emblem-ok-symbolic" pixelSize={16} />
