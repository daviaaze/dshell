  ...
  return (
    <Gtk.Button
      $={(self) => {
        if (set) return
        set = true
        const icon = <Gtk.Image iconName={application.iconName || ""} pixelSize={48} />
        const label = <Gtk.Label wrap cssClasses={["title-2"]} label={application.name} xalign={0} />
        const desc = (
          <Gtk.Label
            cssClasses={["body"]}
        ...
        const textBox = <Gtk.Box orientation={Gtk.Orientation.VERTICAL}>{label}{desc}</Gtk.Box>
        const box = <Gtk.Box spacing={8}>{icon}{textBox}</Gtk.Box>
        self.child = box
      }}
