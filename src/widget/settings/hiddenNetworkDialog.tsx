...
export function showHiddenNetworkDialog(parent: Gtk.Widget) {
    ...
    const connect = () => {
        ...
        if (!name) { setErrorMsg('Network name is required'); return; }
        setConnecting(true);
        setErrorMsg(null);

        const network = Network.get_default();
        const wifi = network.wifi;
        if (!wifi) { setErrorMsg('No WiFi device available'); setConnecting(false); return; }
                ...
                <Adw.PreferencesPage>
                    ...
                    <Gtk.Box orientation={Gtk.Orientation.VERTICAL} spacing={8}>
                        <Gtk.Button
                            ...
                            onClicked={connect}
                        />
                        <Gtk.Button hexpand label="Cancel" onClicked={() => dialog.close()} />
                    </Gtk.Box>
