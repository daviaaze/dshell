...
function drawBoundaryForMonitor(
  ...
  if (!rectsOverlap(geometry.x, geometry.y, geometry.width, geometry.height, mx, my, mw, mh)) {
    return
  }
...
export default () => {
  ...
  const parseColor = (hex: string): { r: number; g: number; b: number; a: number } => {
    const h = hex.replace("#", "")
          ...
          <Gtk.DrawingArea
            $={(self) => {
              self.setDrawFunc((_area, cr, _w, _h) => {
                ...
                drawBoundaryForMonitor(cr, monitor, geom, defaultColor, BORDER_WIDTH)
              })
            }}
            hexpand
            vexpand
