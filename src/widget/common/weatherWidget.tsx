...
export const WeatherIcon = () => {
  ...
  return (
    <IconInfoRow
      icon={createBinding(weather, "info").as((w) => w?.get_icon_name() ?? "")}
      primary={createBinding(weather, "info").as((w) =>
        w?.is_valid() ? w.get_temp_summary() : "—",
      )}
      secondary={createBinding(weather, "info").as(
        (w) => w?.get_location_name() ?? "",
      )}
    />
  )
}
...
export const Weather = () => {
  ...
  const InfoBox = () => (
    <Gtk.Box hexpand spacing={4} orientation={Gtk.Orientation.VERTICAL}>
      <Gtk.Label
        cssClasses={["heading"]}
      ...
      <Gtk.Label
        cssClasses={["title-3"]}
        halign={Gtk.Align.END}
        label={weatherInfo.as((w) => (w?.is_valid() ? w.get_sky() : ""))}
      />
      <Gtk.Label
        halign={Gtk.Align.END}
        label={weatherInfo.as((w) =>
