...
function getServiceDescriptors(s: ReturnType<typeof useSettings>): ServiceDescriptor[] {
  return [
    {
      name: "Weather",
    ...
    {
      name: "ColorScheme",
      init: () => ColorScheme.get_default().init(Weather.get_default(), s.general),
    },
    {
      name: "Inhibit",
    ...
    {
      name: "NightLight",
      init: () => NightLight.get_default().init(s.general, ColorScheme.get_default()),
    },
    {
      name: "Hypridle",
...
export const widgets = () => {
  ...
  for (const { name, init } of getServiceDescriptors(s)) {
    ...
    try {
      init()
      logger.info("mount", `service ${name} init in ${perf.stop(`service-${name}`, "mount").toFixed(1)}ms`)
    } catch (e) {
